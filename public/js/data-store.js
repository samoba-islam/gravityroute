...
const initDataStore = () => {
    if (window.Alpine && !Alpine.store('data')) {
        Alpine.store('data', {
        accounts: [],
        ...
        async refreshModels() {
            ...
            const data = await response.json();
            // Synchronize full account, quota, and model data
        ...
        getModelFamily(modelId) {
            ...
            // Check if model belongs to any configured custom provider
            const isCustom = (this.accounts || []).some(a =>
                a.source === 'custom' && (
                    lower.startsWith(`${(a.name || '').toLowerCase()}-`) ||
                    (a.models || []).some(m => `${(a.name || '').toLowerCase()}-${m.toLowerCase()}` === lower || m.toLowerCase() === lower)
                )
            );
...
if (window.Alpine) {
    ...
    document.addEventListener('alpine:init', initDataStore);
}
