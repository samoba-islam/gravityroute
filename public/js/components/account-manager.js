...
window.Components.accountManager = () => ({
    ...
    // Custom Provider Edit & Top-Up Dialog
    customProviderDialog: {
        provider: null,
        saving: false,
        topUpLoading: false,
        customTopUpAmount: 10,
        customTopUpTokens: 1000000,
        baseUrl: '',
        apiKey: '',
        showApiKey: false,
        pricing: {
    ...
    getMainModelQuota(account) {
        if (account.source === 'custom') {
            ...
            if (account.quotaType === 'dollar') {
                ...
                percent = init > 0 ? Math.max(0, Math.min(100, Math.round((curr / init) * 100))) : 100;
            ...
            return {
                percent,
                model: (account.models && account.models[0]) ? `${account.name}-${account.models[0]}` : (account.name || 'custom')
            };
        }
    ...
    /**
     * Format token numbers (e.g., 1000000 -> 1M)
     */
    formatTokens(num) {
    ...
    /**
     * Open custom provider top-up and edit modal
     */
    openCustomProviderModal(account) {
        this.customProviderDialog.provider = account;
        this.customProviderDialog.saving = false;
        this.customProviderDialog.topUpLoading = false;
        this.customProviderDialog.customTopUpAmount = 10;
        this.customProviderDialog.customTopUpTokens = 1000000;
        this.customProviderDialog.baseUrl = account.baseUrl || '';
        this.customProviderDialog.apiKey = account.apiKey || '';
        this.customProviderDialog.showApiKey = false;
        this.customProviderDialog.pricing = {
            inputPricePerM: account.pricing?.inputPricePerM || 0,
            outputPricePerM: account.pricing?.outputPricePerM || 0,
            cachePricePerM: account.pricing?.cachePricePerM || 0
        };
        this.customProviderDialog.models = [ ...(account.models || []) ];
        this.customProviderDialog.newModelInput = '';

        const modal = document.getElementById('custom_provider_modal');
        if (modal) modal.showModal();
    },

    addCustomModel() {
        const val = (this.customProviderDialog.newModelInput || '').trim();
        if (val && !this.customProviderDialog.models.includes(val)) {
            this.customProviderDialog.models.push(val);
            this.customProviderDialog.newModelInput = '';
        }
    },

    removeCustomModel(idx) {
        this.customProviderDialog.models.splice(idx, 1);
    },

    async topUpCustomBalance(amount) {
        const p = this.customProviderDialog.provider;
        if (!p) return;
        const addAmount = Number(amount);
        if (isNaN(addAmount) || addAmount <= 0) return;

        this.customProviderDialog.topUpLoading = true;
        const store = Alpine.store('global');
        try {
            const { response, newPassword } = await window.utils.request(
                `/api/providers/custom/${encodeURIComponent(p.name || p.email)}`,
                {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ topUpBalance: addAmount })
                },
                store.webuiPassword
            );
            if (newPassword) store.webuiPassword = newPassword;
            const data = await response.json();
            if (data.status === 'ok') {
                store.showToast(`Successfully added $${addAmount.toFixed(2)} to ${p.name}`, 'success');
                await Alpine.store('data').fetchData();
                const refreshed = Alpine.store('data').accounts.find(a => a.name === p.name || a.email === p.email);
                if (refreshed) {
                    this.customProviderDialog.provider = refreshed;
                }
            } else {
                throw new Error(data.error || 'Failed to top up balance');
            }
        } catch (err) {
    ...
    async resetCustomTokens() {
        ...
        try {
            ...
            const data = await response.json();
            if (data.status === 'ok') {
                store.showToast(`Reset token quota for ${p.name}`, 'success');
                await Alpine.store('data').fetchData();
                const refreshed = Alpine.store('data').accounts.find(a => a.name === p.name || a.email === p.email);
                if (refreshed) {
                    this.customProviderDialog.provider = refreshed;
    ...
    async saveCustomProvider() {
        ...
        this.customProviderDialog.saving = true;
        const store = Alpine.store('global');
        try {
            const { response, newPassword } = await window.utils.request(
                `/api/providers/custom/${encodeURIComponent(p.name || p.email)}`,
                {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        baseUrl: this.customProviderDialog.baseUrl,
                        apiKey: this.customProviderDialog.apiKey,
                        pricing: this.customProviderDialog.pricing,
                        models: this.customProviderDialog.models
                    })
                },
                store.webuiPassword
            );
            ...
            const data = await response.json();
            if (data.status === 'ok') {
                store.showToast(`Saved settings for ${p.name}`, 'success');
                await Alpine.store('data').fetchData();
                const modal = document.getElementById('custom_provider_modal');
