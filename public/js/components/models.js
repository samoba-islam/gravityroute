...
window.Components.models = () => ({
    ...
    // Refreshing state for manual model re-fetch
    refreshingModels: false,

    /**
     * Manually re-fetch models from Antigravity server
     */
    async fetchModels() {
        ...
        try {
            const result = await dataStore.refreshModels();
            const count = result.count || dataStore.models?.length || 0;
            store.showToast(
                store.t('modelsFetchedSuccess', { count }) || `Successfully fetched ${count} models from server`,
                'success'
            );
        } catch (e) {
            console.error('Failed to fetch models:', e);
            store.showToast(
                (store.t('modelsFetchFailed') || 'Failed to fetch models') + ': ' + e.message,
                'error'
            );
        } finally {
