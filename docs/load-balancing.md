...
```bash
gravityroute start --strategy=hybrid       # Default: smart distribution (or: acc start)
gravityroute start --strategy=sticky       # Cache-optimized
gravityroute start --strategy=round-robin  # Load-balanced
```

**Or via WebUI:** Settings → Server Settings → Account Selection Strategy
...
```bash
# List all accounts
gravityroute accounts list

# Verify account health
gravityroute accounts verify
