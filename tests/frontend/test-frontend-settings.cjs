...
const tests = [
    ...
    {
        name: 'Settings view has WebUI Auth section',
        async run() {
            const res = await request('/views/settings.html');
            const html = res.data;

            if (!html.includes('webuiAuthConfig')) {
                throw new Error('webuiAuthConfig component not found in settings');
            }
            if (!html.includes('settingsTab === \'auth\'')) {
                throw new Error('WebUI Auth tab not found in settings');
            }
            if (!html.includes('toggleAuth')) {
                throw new Error('toggleAuth function not found in settings');
            }
            if (!html.includes('saveCredentials')) {
                throw new Error('saveCredentials function not found in settings');
            }
            return 'WebUI Auth tab and controls present';
        }
    },
