...
const BASE_CONFIG_DIR = join(process.env.HOME || process.env.USERPROFILE || '', '.config');
const NEW_CONFIG_DIR = join(BASE_CONFIG_DIR, 'gravityroute');
const LEGACY_CONFIG_DIR = join(BASE_CONFIG_DIR, 'antigravity-proxy');
const CONFIG_DIR = existsSync(NEW_CONFIG_DIR) ? NEW_CONFIG_DIR : (existsSync(LEGACY_CONFIG_DIR) ? LEGACY_CONFIG_DIR : NEW_CONFIG_DIR);
...
function showHelp() {
  console.log(`
╭${'─'.repeat(58)}╮
│  🚀 GravityRoute v${packageJson.version.padEnd(38)}│
╰${'─'.repeat(58)}╯

Route Claude Code CLI through intelligent proxy routing
with multi-model API and load balancing across accounts.

USAGE
  gravityroute <command> [options]
  gravityRoute <command> [options]
...
Docs: https://github.com/samoba-islam/gravityroute
`);
}
