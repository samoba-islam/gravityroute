 ...
 * Respects user visibility preferences by skipping models marked as hidden in Settings -> Models
 *
...
export async function listModels(token) {
    ...
    const modelList = supportedEntries
        .filter(([modelId]) => {
            // Skip models explicitly hidden in WebUI settings
            if (modelMapping[modelId]?.hidden === true) {
                return false;
            }
            return true;
        })
