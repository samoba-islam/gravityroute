...
import { createOpenAiHandlers } from './openai/routes.js';
import { dispatchMessagesRequest } from './providers/custom-dispatcher.js';
...
apiKeyManager.setAccountManager(accountManager);
...
async function ensureInitialized() {
    ...
    initPromise = (async () => {
        try {
            ...
            logger.success(`[Server] Account pool initialized: ${status.total} total, ${status.available} available, ${status.rateLimited} rate-limited, ${status.invalid} invalid`);
        } catch (error) {
            initError = error;
...
// API Key authentication middleware for /v1/*, /chat/completions, and related endpoints
app.use(['/v1', '/chat/completions', '/models', '/embeddings'], (req, res, next) => {
    ...
    // Skip validation if neither master key nor custom keys are configured
    if (!masterKeyConfigured && !hasCustomKeys) {
        return next();
    }

    // 1. Check master key first if configured
    ...
    // 2. Check custom keys if any exist
    if (hasCustomKeys && providedKey) {
        ...
        if (validation.valid) {
            ...
            logger.warn(`[API] API Key rejected (${validation.reason}) from ${clientIp}`);
    ...
    logger.warn(`[API] Unauthorized request from ${req.ip}, invalid or missing API key`);
    if (isOpenAiReq) {
 ...
 */
// TEMPORARY DEBUG: Dump raw model catalog data
app.get('/debug/raw-models', async (req, res) => {
    try {
        await ensureInitialized();
        const { account } = accountManager.selectAccount();
        if (!account) return res.status(503).json({ error: 'No accounts available' });
        const token = await accountManager.getTokenForAccount(account);
        const projectId = account.subscription?.projectId || null;
        const rawData = await fetchAvailableModels(token, projectId);
        
        // Filter to only 3.1 models for readability
        const filtered = {};
        if (rawData && rawData.models) {
            for (const [id, data] of Object.entries(rawData.models)) {
                if (id.includes('3.1')) {
                    filtered[id] = data;
                }
            }
        }
        res.json({ allModelIds: Object.keys(rawData.models || {}), filtered, raw_gemini_3_1_pro_high: rawData.models?.['gemini-3.1-pro-high'], raw_gemini_3_1_pro_low: rawData.models?.['gemini-3.1-pro-low'] });
    } catch (error) {
...
app.get('/account-limits', async (req, res) => {
    try {
        ...
        const results = await Promise.allSettled(
            allAccounts.map(async (account) => {
                ...
                if (account.source === 'custom') {
                    ...
                    if (account.quotaType === 'dollar') {
                        const init = Number(account.initialBalance) || 0;
                        const curr = account.currentBalance !== undefined ? Number(account.currentBalance) : init;
                        remainingFraction = init > 0 ? Math.max(0, Math.min(1, curr / init)) : 1;
                    } else if (account.quotaType === 'token') {
                        const init = Number(account.initialTokens) || 0;
                        const curr = account.currentTokens !== undefined ? Number(account.currentTokens) : init;
                        remainingFraction = init > 0 ? Math.max(0, Math.min(1, curr / init)) : 1;
                    }

                    const customModels = {};
                    for (const rawModel of (account.models || [])) {
                        const fullModelId = `${account.name}-${rawModel}`;
                        customModels[fullModelId] = {
                            remainingFraction,
                            resetTime: null
                        };
                    }

                    const isExhausted = (account.quotaType === 'dollar' && (account.currentBalance || 0) <= 0) ||
                                        (account.quotaType === 'token' && (account.currentTokens || 0) <= 0);

                    return {
                        email: account.email,
                        status: isExhausted ? 'exhausted' : (account.enabled === false ? 'disabled' : 'ok'),
                        subscription: { tier: (account.type || 'custom').toUpperCase(), projectId: null },
                        models: customModels
                    };
                }
        ...
        if (accountManager) {
            accountManager.latestAvailableModels = sortedModels;
        }
        ...
        const responseData = {
            ...
            accounts: accountLimits.map(acc => {
                ...
                return {
                    ...
                    quotaType: metadata.quotaType || null,
                    initialBalance: metadata.initialBalance !== undefined ? metadata.initialBalance : null,
                    currentBalance: metadata.currentBalance !== undefined ? metadata.currentBalance : null,
                    initialTokens: metadata.initialTokens !== undefined ? metadata.initialTokens : null,
                    currentTokens: metadata.currentTokens !== undefined ? metadata.currentTokens : null,
...
// Initialize OpenAI compatibility handlers
...
// Chat Completions
app.post('/v1/chat/completions', openAiHandlers.handleChatCompletions);
app.post('/chat/completions', openAiHandlers.handleChatCompletions);

// Models
app.get('/v1/models', openAiHandlers.handleGetModels);
app.get('/models', openAiHandlers.handleGetModels);
app.get('/v1/models/*model', openAiHandlers.handleGetSingleModel);
app.get('/models/*model', openAiHandlers.handleGetSingleModel);

// Embeddings
app.post('/v1/embeddings', openAiHandlers.handleEmbeddings);
app.post('/embeddings', openAiHandlers.handleEmbeddings);
...
app.post('/v1/messages/count_tokens', (req, res) => {
    if (req.apiKeyRecord?.modelRestrictionEnabled) {
        ...
        const check = apiKeyManager.isModelAllowed(req.apiKeyRecord, requestedModel);
        if (!check.allowed) {
            return res.status(403).json({
                type: 'error',
                error: {
                    type: 'permission_error',
                    message: check.reason || `Model '${requestedModel}' is not allowed for this API key.`
                }
            });
        }
    }
...
app.post('/v1/messages', async (req, res) => {
    try {
        ...
        const clientRequestedModel = requestedModel;
        ...
        // Check API key model access permissions
        if (req.apiKeyRecord?.modelRestrictionEnabled) {
            const checkClient = apiKeyManager.isModelAllowed(req.apiKeyRecord, clientRequestedModel);
            const checkTarget = apiKeyManager.isModelAllowed(req.apiKeyRecord, modelId);
            if (!checkClient.allowed && !checkTarget.allowed) {
                logger.warn(`[APIKeys] Key "${req.apiKeyRecord.name}" (${req.apiKeyRecord.id}) blocked from calling model "${clientRequestedModel}"`);
                return res.status(403).json({
                    type: 'error',
                    error: {
                        type: 'permission_error',
                        message: checkClient.reason || `Model '${clientRequestedModel}' is not allowed for this API key. Allowed models: ${(req.apiKeyRecord.allowedModels || []).join(', ')}`
                    }
                });
            }
        }

        // Check if requested model targets a custom provider ({provider_name}-{model_name})
        const customProviderMatch = accountManager.findCustomProviderForModel(modelId);
        if (customProviderMatch) {
            return await dispatchMessagesRequest(
                customProviderMatch.provider,
                customProviderMatch.rawModel,
                req.body,
                res,
                accountManager
            );
        ...
        if (stream) {
            ...
            try {
                ...
                let streamCacheTokens = 0;

                const extractUsage = (usage) => {
                    if (!usage) return;
                    if (usage.input_tokens) streamInputTokens = Math.max(streamInputTokens, usage.input_tokens);
                    if (usage.output_tokens) streamOutputTokens = Math.max(streamOutputTokens, usage.output_tokens);
                    const cache = (usage.cache_read_input_tokens || 0) + (usage.cache_creation_input_tokens || 0);
                    if (cache) streamCacheTokens = Math.max(streamCacheTokens, cache);
                };

                // If the generator isn't done, send the first chunk
                if (!firstResult.done) {
                    extractUsage(firstResult.value?.message?.usage || firstResult.value?.usage);
                    if (firstResult.value?.message && clientRequestedModel === 'gemini-3.1-pro-high') {
                        firstResult.value.message.model = clientRequestedModel;
                    }
                ...
                for await (const event of generator) {
                    extractUsage(event.message?.usage || event.usage);
                    if (event?.message && clientRequestedModel === 'gemini-3.1-pro-high') {
                        event.message.model = clientRequestedModel;
                    }
            ...
            if (response && clientRequestedModel === 'gemini-3.1-pro-high') {
                response.model = clientRequestedModel;
            }
            if (req.apiKeyRecord) {
                const inputTokens = response.usage?.input_tokens || 0;
                const outputTokens = response.usage?.output_tokens || 0;
                const cacheTokens = (response.usage?.cache_read_input_tokens || 0) + (response.usage?.cache_creation_input_tokens || 0);
                apiKeyManager.recordDetailedUsage({
                    keyId: req.apiKeyRecord.id,
                    model: request.model,
                    inputTokens,
